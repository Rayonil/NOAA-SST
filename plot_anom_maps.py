# Plotar a Média Mensal da Anômalia Temperatura da Superfície do Mar
...
for file in input_dir:
    ...
    # Define os níveis (boundaries) incluindo -5 até 5
    levels = np.arange(-5, 6, 0.5)  # Isso gera 11 limites => 10 intervalos
    # Cores: branco no intervalo que contém o 0
    # Usaremos a paleta RdBu_r que é centrada no 0 e remapearemos o branco no centro
    # Gera cores base (sem branco)
    base_cmap = plt.cm.RdYlBu_r(np.linspace(0, 1, len(levels)-1))
    
    # Substitui o intervalo central (onde está o 0) por branco
    mid_idx = len(base_cmap) // 2
    base_cmap[mid_idx] = [1, 1, 1, 1]  # RGBA para branco
    base_cmap[9] = [1, 1, 1, 1]  # RGBA para branco
    
    # Cria cmap e norm
    cmap = ListedColormap(base_cmap)
    norm = BoundaryNorm(boundaries=levels, ncolors=len(base_cmap))
...
# ======================================================================================================================================= 
    ...
    nome_saida = f"Anom_{data_plot}"
    caminho_saida = os.path.join(year_dir, nome_saida)
    plt.savefig(caminho_saida, dpi=400)
    print(f"✅ Plot Anomomalia da SST mensal de {data_plot} salva: {caminho_saida}")
plt.show()